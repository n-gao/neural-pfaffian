...
Taken from: https://github.com/google-deepmind/ferminet/tree/main"""
...
def cart2sph(r: jax.Array) -> tuple[jax.Array, jax.Array, jax.Array]:
...
def cartesian_product(r: jax.Array, ell: int) -> jax.Array:
    """Computes all cartesian products of order ell."""
    # i.e. all terms (x^a * y^b * z^c) for a+b+c = ell
    if ell == 0:
        return jnp.array([1.0])
    return jnp.prod(
        jnp.stack(jnp.meshgrid(*[r] * ell, indexing='ij'), axis=0).reshape(ell, -1),
        axis=0,
    )


def solid_harmonic(r: jax.Array, l_max: int) -> jax.Array:
...
def solid_harmonic_from_cart(r: jax.Array, l_max: int) -> jax.Array:
    """Computes all solid harmonics r**ell Y_{ell, m} for all ell <= l_max.
